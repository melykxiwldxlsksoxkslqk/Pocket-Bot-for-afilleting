...
from app.core.dispatcher import admin_panel  # Используем единый экземпляр из диспетчера
...
class AdminCheckMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if isinstance(event, Message):
            await event.answer("Извините, эта функция доступна только администраторам.")
        elif isinstance(event, CallbackQuery):
            await event.answer("Извините, эта функция доступна только администраторам.", show_alert=True)
